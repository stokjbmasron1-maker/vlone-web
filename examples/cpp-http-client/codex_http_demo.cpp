...
static const wchar_t* kHost = L"codex-web.vercel.app";
...
static bool HttpsPostJson(const wchar_t* host, const wchar_t* path, const std::string& bodyUtf8,
  ...
  HINTERNET hSession =
      WinHttpOpen(L"CodeX-Client/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME,
...
static std::string FallbackHwidFromPcName() {
  ...
  if (!GetComputerNameW(buf, &sz) || sz == 0) return "codex-pc-unknown";
...
static int RunMainBody() {
  ...
  if (valid) {
    ...
    OutLn("");
    OutLn("Opening mod checklist window (saved under Local AppData\\CodeX)...");
    ShowModPreferenceWindow(GetConsoleWindow());
  } else {
